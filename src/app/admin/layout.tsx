...
export default function AdminLayout({
  ...
  return (
    <div className="min-h-screen bg-stone-100">
      {/* Sidebar */}
      <aside
        className={`fixed left-0 top-0 h-full bg-white border-r border-stone-200 flex flex-col shadow-sm transition-all duration-300 ${
          sidebarCollapsed ? "w-20" : "w-72"
        }`}
      >
        {/* Logo & Brand */}
        <div className={`border-b border-stone-100 ${sidebarCollapsed ? "p-4" : "p-6"}`}>
          {!sidebarCollapsed && (
            <Link
              href="/"
          ...
          {sidebarCollapsed ? (
            <>
              ...
              <button
                onClick={() => signOut()}
                title="Sign Out"
                className="w-full flex items-center justify-center p-2.5 text-stone-500 hover:text-red-600 hover:bg-red-50 rounded-xl transition-colors"
              >
                <LogOut className="w-4 h-4" />
              </button>
            </>
        ...
        <button
          ...
          className="absolute top-4 -right-3 w-6 h-6 bg-white border border-stone-200 rounded-full shadow-sm flex items-center justify-center text-stone-400 hover:text-stone-600 hover:bg-stone-50 transition-colors"
